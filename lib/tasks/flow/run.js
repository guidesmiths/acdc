...
module.exports = {
    fn: function run(input, ctx, cb) {
        setImmediate(function() {
            validate(input, ctx, module.exports.schema, function(err) {
                if (err) return cb(err)

                var taskCtx = getTaskCtx(ctx)

                validate(input, taskCtx, ctx.params.task.schema || Joi.any(), function(err) {
                    if (err) return cb(err)
                    runTask(taskCtx)
                })
            })
        ...
        function validate(input, ctx, schema, cb) {
            debug('Validating %s context', ctx.name || 'run')
